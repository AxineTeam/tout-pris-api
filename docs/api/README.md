...
**La création d'un voyage accepte `participants` et `kits`, deux listes d'identifiants facultatives**, alors que le reste de l'API est granulaire. Ce n'est pas un raccourci de confort : enchaînés par le client, ces appels ont un ordre contraint dont la violation est silencieuse. Qui embarque un kit avant d'ajouter les participants perd toutes les lignes du kit qui visent une personne — `instantiate_kit` les ignore — et reçoit un `201` parfaitement normal sur un voyage à moitié préparé. Côté serveur, les participants sont créés avant l'instanciation des kits, une fois pour toutes. L'appel unique est de surcroît une transaction : un foyer sans statut répond `409` sans laisser derrière lui le voyage vide dont personne n'a voulu. `build_trip` vit dans `trips/preparation.py` à côté de `duplicate_trip`, qui fait déjà exactement cela.

Les deux listes étant facultatives, un corps réduit à `{"name", "date"}` crée le voyage comme avant, et les routes granulaires restent : on ajoute un participant ou on embarque un kit à tout moment ensuite. Un identifiant venu d'un autre foyer répond `404` comme partout ailleurs, par le même `HouseholdScopedRelation`, et rien n'est créé. Un identifiant répété dans l'une des deux listes répond `400` et non le `409` du second `POST` sur les participants : le doublon est ici dans le corps même de la requête, il n'entre en conflit avec rien d'existant. La réponse est le voyage à l'unité, participants et lignes développés, pour que le client atterrisse sur l'écran fini sans relire.
...
**`POST /api/households/{household_id}/trips/{trip_id}/kits/` instancie un kit dans le voyage et renvoie les lignes créées**, `201` quand il en a créé, `200` quand il n'en a créé aucune. Le tableau vide est le seul signal dont le front dispose pour dire « ce kit n'a rien ajouté » au moment du clic : aucune table ne garde trace d'un kit choisi, et recharger le voyage ne le distinguerait pas d'un kit qui n'aurait ajouté que des lignes déjà présentes. Les règles de la copie — ligne ignorée si elle vise un non-participant, ligne considérée présente au même objet et à la même personne, quantité jamais réécrite — sont dans [`docs/model/trips.md`](../model/trips.md), et elles vivent dans `trips/preparation.py` plutôt que dans la vue, comme `catalog/statuses.py` porte les règles de statut : la vue reçoit, valide, délègue.
...
**Vider n'est pas un geste unique, c'est la colonne qui décide.** `person` sur une ligne de kit est nullable et se vide avec `null` ; `description` est une chaîne facultative et se vide avec `""`, un `null` y répondant `400`. `openapi.yaml` le dit champ par champ avec `nullable`.

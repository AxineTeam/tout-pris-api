...
Backend Django for Tout Pris, with [django-ninja](https://django-ninja.dev) for the API.
...
```bash
uv run python manage.py export_openapi_schema --api tout_pris.api.api --output openapi.json --indent 2
```
...
- `GET /api/docs` — interactive documentation generated by django-ninja
- `GET /api/openapi.json` — OpenAPI schema served by the running app
- `/admin/` — Django admin

## OpenAPI

django-ninja generates the schema from the code. It is committed as [`openapi.json`](openapi.json) and regenerated with the `export_openapi_schema` command above. CI fails if the committed file drifts from the code, so regenerate it whenever routes or schemas change.
